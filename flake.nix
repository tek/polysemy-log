{
  ...
  inputs = {
    ...
    conc.url = github:tek/polysemy-conc;
    polysemy-time.url = github:tek/polysemy-time;
    polysemy-test.follows = "conc/polysemy-test";
  };

  outputs = { tryp-hs, polysemy-time, polysemy-test, conc, ... }@inputs:
  let
    common = { hackage, source, jailbreak, ... }: {
      ...
      polysemy-conc = source.sub conc "packages/conc";
